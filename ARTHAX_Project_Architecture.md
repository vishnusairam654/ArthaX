...
* **Animation:** Full motion/3D stack specified in Section 5.8 (GSAP, Motion, Anime.js, Vanta.js, Three.js). Prioritize physical, precise, and soft motion — restraint is the default, not the exception.
...
## **8\. Motion, 3D & Component Library Stack**

Each library has a defined role so animation reinforces "Calm, Precise, Human" rather than competing with it. The ecosystem should never read as a demo reel.

**Orchestration**

* **GSAP \+ ScrollTrigger:** Primary motion engine for portal-level sequences — page-load reveals, scroll-triggered moments on the Central Guide Board, cross-portal transition timelines.  
* **Motion (Framer Motion):** Component-level declarative transitions inside React — modal/drawer enter-exit, list reordering (transaction feeds, order book updates), tab transitions within a portal.

**Micro-interaction**

* **Anime.js:** Lightweight, one-off details only — balance count-up animations, button press feedback, animating the Artha/Flow SVG motif on loading and empty states. Reserve for moments where GSAP's full timeline system is overkill.

**Ambient / 3D — restrained use only**

* **Vanta.js:** Ambient WebGL backgrounds, appropriate only on low-stakes, non-transactional surfaces — the Central Guide Board hero, possibly the Shop storefront. Keep off Central Bank, Bank, User, and Stock portals; an animated background behind a balance sheet undercuts trust faster than almost anything else in this system.  
* **Three.js:** Reserved for genuinely 3D moments — the Shop's avatar/pet/frame previews. Not used for financial data visualization; market charts and portfolio data stay flat and precise (Recharts/D3-class tooling), since 3D adds interpretive noise to numbers people are trusting with real money.

**Component foundations — customize, don't ship default**

* **Shadcn/ui:** Base primitives for `@arthax/ui`. Every component must be re-themed through the design token system (Sections 5.2, 5.4, 5.5) before use — unmodified Shadcn defaults are one of the clearest signals that a UI was AI-scaffolded rather than designed, which runs directly against this document's own "Distinctive, not Template-like" principle.  
* **React Bits / Kokonut UI:** Use as reference libraries for interaction patterns (text reveals, animated empty states, layout ideas) — not as drop-in components. Extract the pattern, rebuild it inside the ARTHAX token system, same discipline as Shadcn above.

**Guardrail:** This is a wide library surface for one system. No single screen should combine more than two or three of these at once. A portal that layers GSAP timelines, a Vanta background, a Three.js preview, and unmodified component-library cards simultaneously will read as over-produced rather than trustworthy. Motion and 3D usage per screen is a Design Director decision made against the token system, not a library added because it's available.
